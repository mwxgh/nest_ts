...
export class RoleController {
  ...
  async createRole(@Body() data: CreateRoleDto): Promise<CreateResponse> {
    const role = await this.role.createRole(data)
  ...
  async readRoles(
    ...
    if (query.perPage || query.page) {
      ...
      return this.response.paginate(
        await this.role.paginationCalculate(queryBuilder, paginateOption),
  ...
  async readRole(
    ...
    const role: RoleEntity = await this.role.findOneOrFail(id, {
      relations: this.relations,
    })
  ...
  async updateRole(
    ...
    const role = await this.role.updateRole({ id, data })
  ...
  async deleteRole(
    ...
    await this.role.deleteRole({ id, currentUser })
